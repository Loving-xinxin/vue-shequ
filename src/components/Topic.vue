      ...
      <ul>
        <li v-for=" comment in topic.replies" :key="comment.id">
          ...
          <div>
            ...
            <span>{{myMoment(comment.create_at)}}</span>
            <span v-html="comment.content" class="content"></span>
            <span>
...
export default {
  ...
  methods: {
    ...
    isUped(id) {
      //刚进页面的时候看评论是否被用户点赞了;点赞的动作反应在返回数据的 action 字段中，up or down。
      return this.topic.replies
        .find(item => item.id === id)
        .ups.indexOf(sessionStorage.getItem("user_id"));
    },
    addreply(loginname) {
      this.text = `@${loginname}`;
...
.topic .article-head > div > .collect {
...
.comment ul li div > span:nth-child(2) {
  color: #000;
  font-weight: bold;
  margin-left: 10px;
}
.comment ul li div > span:nth-child(3) {
  color: #08c;
}
